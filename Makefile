...
ROOT_DIR := $(HOME)
CONTEXTO = "Vamos de Monitoração Web Pra Dentro... | Versão: "$(shell cat .version)
...
#CONSTRUIR IMAGEM
.PHONY: monitoracao_build
monitoracao_build: ## Constroi o container da Monitoracao web
		@clear
		make git_trazer
		@$(call msg_warn,"Espere que irei fazer uma build do container \(novinho em folha\)...")
		docker build --build-arg VAR_VERSAO=$(shell cat .version) -t docker-monitoracao:$(shell cat .version) $(CURRENT_DIR)/app/

#Usar o monitoracao web
.PHONY: monitoracao
monitoracao: ## Exeutar localmente o monitoramento via docker
		@clear
		make monitoracao_build
		@docker run -it --rm -e "COMPLEMENTO=Muito" docker-monitoracao:$(shell cat .version)

#TESTAR IMAGEM
.PHONY: monitoracao_testa_build
